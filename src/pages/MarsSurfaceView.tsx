          ...
          {activeTab === 'satellite' && (
            <div className="space-y-6 animate-in fade-in">
                ...
                <div className="flex items-center gap-2 flex-wrap shrink-0">
                  {surfaceData.googleEarthUrl && (
                    ...
                    >
                      ...
                      <span>
                        {surfaceData.bodyType === 'earth' ? 'Launch Google Earth 3D' : 'Launch 3D Explorer'}
                      </span>
              ...
              <div className="relative w-full h-[380px] sm:h-[480px] lg:h-[560px] rounded-2xl overflow-hidden border border-slate-700 bg-space-950 shadow-2xl">
                <iframe
                  title={`${surfaceData.name} Google Satellite / 3D Map`}
                  src={
                    surfaceData.googleMapsEmbedUrl ||
                    `https://maps.google.com/maps?q=${encodeURIComponent(surfaceData.coordinates)}&t=k&z=15&ie=UTF8&iwloc=&output=embed`
                  }
                ...
                {/* Live Satellite Layer HUD Overlay */}
                <div className="absolute top-3 left-3 px-3 py-1.5 rounded-xl bg-space-950/90 backdrop-blur-md border border-slate-700/80 text-[10px] sm:text-xs font-mono text-emerald-300 flex items-center gap-2 pointer-events-none shadow-xl">
                  <span className="w-2 h-2 rounded-full bg-emerald-400 animate-ping" />
                  <span className="font-bold">
                    {surfaceData.bodyType === 'earth' ? 'GOOGLE SATELLITE & USGS ORTHO' : 'NASA PLANETARY SATELLITE'}
                  </span>
                  <span className="text-slate-600">|</span>
                  <span className="text-slate-300 hidden sm:inline">{surfaceData.coordinates}</span>
                </div>

                <div className="absolute bottom-3 right-3 px-3 py-1.5 rounded-xl bg-space-950/90 backdrop-blur-md border border-slate-700/80 text-[10px] font-mono text-slate-400 pointer-events-none shadow-xl hidden sm:flex items-center gap-2">
                  <span>Pan & Zoom Enabled</span>
                  <span className="text-slate-600">·</span>
                  <span className="text-cyan-400 font-bold">Tilt in Google Earth 3D for mesh view</span>
                </div>
              </div>
